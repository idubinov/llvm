...
__SYCL_INLINE namespace cl {
  ...
  private:
    ...
    /* Helper class for conversion operator, used instead void. User cannot even
     * try to get address of the operator __private_class(). User may try to get
     * an address of operator void() and will get the compile-time error */
  ...
  public:
     ...
     * conversion:
     * int a = id<1>(value); */
    operator detail::conditional_t<(dimensions == 1), size_t, __private_class>()
        const {
...
#define __SYCL_GEN_OPT(op)                                                     \
  template <typename T>                                                        \
  detail::enable_if_t<std::is_integral<T>::value && (dimensions == 1), bool>   \
  ...
  template <typename T>                                                        \
  friend detail::enable_if_t<std::is_integral<T>::value && (dimensions == 1),  \
                             bool>                                             \
  operator op(const T &lhs, const id<dimensions> &rhs) {                       \
...
#define __SYCL_GEN_OPT(op)                                                     \
  ...
  template <typename T>                                                        \
  detail::enable_if_t<std::is_integral<T>::value, id<dimensions>> operator op( \
      const T &rhs) const {                                                    \
    id<dimensions> result;                                                     \
    for (int i = 0; i < dimensions; ++i) {                                     \
  ...
  template <typename T>                                                        \
  friend detail::enable_if_t<std::is_integral<T>::value, id<dimensions>>       \
  operator op(const T &lhs, const id<dimensions> &rhs) {                       \
    id<dimensions> result;                                                     \
    for (int i = 0; i < dimensions; ++i) {                                     \
